...
import RemovableItem from "../ui/removableItem";
...
type EngineeringContractorFormProps = {
  type: "OFFICE" | "CONTRACTOR";
};

export const EngineeringContractorForm = ({ type }: EngineeringContractorFormProps) => {
  const dir = useLocale() === "ar" ? "rtl" : "ltr";
  const { openModal } = useModalStore();
  const t = useTranslations("auth.register.engineering_contractor");
  ...
  const engineeringContractorSchema = createEngineeringContractorSchema({
    ...
    image: {
      required: t("logo.validation.required"),
      size: t("logo.validation.size"),
      type: t("logo.validation.type"),
    },
  }).pick({
    name: true,
    email: true,
  ...
  const handleFormSubmit = async (data: EngineeringContractorSchema) => {
    ...
    formData.append("commercial_register_number", data.commercialRegister);
    formData.append("type", data.facilityType);
    formData.append(
      "services",
    ...
    try {
      const response = await fetch(
        `https://api.shaghalni.sa/auth/signup/${type === "OFFICE" ? "offices" : "contractors"}`,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6 lg:gap-x-8">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    {type === "OFFICE" ? t("name.engineering_office_label") : t("name.contractor_label")}
                  </FormLabel>
                  <FormControl>
                  ...
                  <div className="relative">
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent dir={dir}>
                        <SelectItem value="1">{t("facility_type.options.company")}</SelectItem>
                        <SelectItem value="2">{t("facility_type.options.organization")}</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                    ...
                    <Select onValueChange={(newValue) => field.onChange([...field.value, newValue])}>
                      <FormControl className="pe-3.5">
                        <>
                          <SelectTrigger
                            dir={dir}
                            className="ps-9 bg-[#F6FBF8] rounded-2.5xl justify-end border-garyClr ring-0"
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <span className="absolute inset-0 transform text-sm bg-[#F6FBF8] text-primaryClr pointer-events-none flex items-center justify-center gap-2 rounded-2.5xl border border-primaryClr">
                          ...
                          {value ? value.name : t("logo.placeholder")}
                        </span>
                      </div>
                    </FormControl>
                    <FormMessage />
